...
(defun nextT (prevT prevP prevA prevB)
(- prevT (* prevP (expt (- prevA (nextA prevA prevB)) 2)))
)
...
(defun piCalc (prevA prevB prevT prevP oldPi)
  (let (
    (nextA (nextA prevA prevB))
    (nextB (nextB prevA prevB))
    (nextP (nextP prevP))
    (nextT (nextT prevT prevP prevA prevB))
    ;;(newPi (roundToPrecision (/ (expt (+ (nextA prevA prevB) (nextB prevA prevB))2) (* 4 (nextT prevT prevP prevA prevB))) 10000))
    )
        (let (
            (newPi (/ (expt (+ nextA nextB)2) (* 4 nextT)))
                ...
                (if
                    (= oldPi newPi)
                    newPi
                    (piCalc nextA nextB nextT nextP newPi)
                )
        )
...
;;(write (coerce (piCalc prevA prevB prevT prevP 0) 'long-float))
(write (coerce (piCalc 1L0 (/ 1L0(sqrt 2L0)) (/ 1L0 4L0) 1L0 0) 'long-float))
